...
object AutoImplantData {

  val autoimplantDir = new File(DataDirectory.dir, "autoimplant")

  val modelName = "gaussianModelMixed-sigma40.0-a4.0"
  val modelDir = new File(autoimplantDir, s"$modelName.h5")

  val modelLmsDir = new File(autoimplantDir, "skull_landmarks.json")
  val modelLmsDirExtra = new File(autoimplantDir, "referenceExtraLandmarks.json")

  val targetDir: File = new File(autoimplantDir, "mesh-initial")
  val targetLandmarksFile = new File(autoimplantDir, "landmarks-initial")

  val gtalignedDir: File = new File(autoimplantDir, "complete-aligned")

  val refLabelMap = new File(autoimplantDir, "refmesh-labeled.vtk")

  val outputDirSeqGPMMautoimplant = new File(autoimplantDir, "outputSeqGPMM")
